...
% content after spiking decon
  ...
  fprintf('size(Ps) = %d, size(f) = %d, min(f) = %f, max(f) = %f\n', size(Ps), size(f), min(f), max(f));
  [Po,f] = smooth_spectrum(o,dt,30,'li');
  fprintf('size(Po) = %d, size(f) = %d, min(f) = %f, max(f) = %f\n', size(Po), size(f), min(f), max(f));
